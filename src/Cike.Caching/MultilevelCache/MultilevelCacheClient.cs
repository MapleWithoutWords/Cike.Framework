namespace Cike.Caching.MultilevelCache;

public class MultilevelCacheClient : MultilevelCacheClientBase, ISingletonDependency
{
        ...
        IFormatCacheKeyProvider? formatCacheKeyProvider)
    {
        ...
        _subscribeKeyType = multilevelCacheOptions.Value.SubscribeKeyType;
        GlobalCacheOptions = new MultilevelCacheOptions()
        {
            CacheKeyType = multilevelCacheOptions.Value.GlobalCacheOptions.CacheKeyType,
            MemoryCacheEntryOptions = multilevelCacheOptions.Value.CacheEntryOptions
        };
        _subscribeKeyPrefix = multilevelCacheOptions.Value.SubscribeKeyPrefix;
        _formatCacheKeyProvider = formatCacheKeyProvider ?? new DefaultFormatCacheKeyProvider();
        _instanceId = multilevelCacheOptions.Value.InstanceId;
    ...
    private static SubscribeValueChangedOptions<T>? GetSubscribeOptions<T>(Action<T?>? valueChanged)
    {
        SubscribeValueChangedOptions<T>? subscribeOptions = null;
        if (valueChanged != null)
        {
            subscribeOptions = new SubscribeValueChangedOptions<T>()
            {
    ...
    private void Subscribe<T>(
        string channel,
        SubscribeValueChangedOptions<T>? options = null)
    {
