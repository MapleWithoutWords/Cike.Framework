namespace Cike.Caching.DistributedCache;

public class RedisCacheClient : RedisCacheClientBase, ISingletonDependency
{
    ...
    public RedisCacheClient(
        IOptions<RedisConfigurationOptions> redisConfigurationOptions,
        IFormatCacheKeyProvider? formatCacheKeyProvider = null,
        //JsonSerializerOptions? jsonSerializerOptions = null,
        ITypeAliasProvider? typeAliasProvider = null)
        : base(redisConfigurationOptions.Value, null)
    {
    ...
    private async Task<List<DataCacheModel<T>>> GetListAsync<T>(List<string> keys)
    {
        ...
        batch.Execute();
        var hashEntries = await Task.WhenAll(tasks).ConfigureAwait(false);
    ...
    {
        var formatCacheKey = FormatCacheKey<T>(key, action);
        return SetCoreAsync(formatCacheKey, value, options);
    }
